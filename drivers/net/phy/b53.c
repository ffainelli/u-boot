 ...
 * supports vendor tags and VLAN configuration that can affect the switching decisions.
 * This driver uses a simple configuration in which all ports are only allowed to
 * send frames to the CPU port and receive frames from the CPU port this providing
 ...
 * switch but the BCM53xx being largely register compatible, extending it to cover
...
static int b53_mdio_read32(struct mii_dev *bus, u8 page, u8 reg, u32 *val)
{
	...
	*val = bus->read(bus, BRCM_PSEUDO_PHY_ADDR, MDIO_DEVAD_NONE, REG_MII_DATA0);
	*val |= bus->read(bus, BRCM_PSEUDO_PHY_ADDR, MDIO_DEVAD_NONE,
			  REG_MII_DATA1) << 16;
